...
**Policy:** `clear` → proceed · `sanctions` → `REJECTED` or `UNDER_REVIEW` per policy · `pep` → `UNDER_REVIEW` · never show hit details to end-users; log for compliance; subscribe to `verification.manual_review_queued`.